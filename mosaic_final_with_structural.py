...
def getDominantRGB(image): 
    ...
    _,_,centers = cv2.kmeans(im,1,None,criteria,10,flags)
...
def getBestMatchIndex(input_avg, avgs, method): 
    """ 
    ...
    for val in avgs:
        if (method == 'mse'):
            ...
            _rw = 2 + (_redmean)/256
            _gw = 4
            _bw = 2 + (255 - _redmean)/256
            
            dist = math.sqrt((_rw*_rd**2)
                             +(_gw*_gd**2)
                             +(_bw*_bd**2))
...
def getBestMatchIndexStruct(img, img_list):
    """ 
    return index of best Image match based on structural distance 
    """

    t_img = np.array(img.convert('RGB'))
    ...
    for _img in img_list:
        ...
        _img = _img.resize(img.size, Image.ANTIALIAS)
        _img = np.array(_img)
        _img = _img[:, :, ::-1].copy()
        
        # print("SHAPE {} {}".format(t_img.shape, _img.shape))
        
        sim = metrics.structural_similarity(t_img, _img)
...
def createPhotomosaic(target_image, input_images, grid_size, reuse_images=True, method='mse'): 
    """ 
    ...
    if (method != 'struct'):
        for img in input_images:
            if (method != 'dom'):
                ...
                avgs.append(getDominantRGB(img))
        for img in target_images: 
	    # target sub-image average
            if (method != 'dom'):
                avg = getAverageRGB(img)
            else:
                avg = getDominantRGB(img)
            # print("AVG is ", avg)
	    # find match index 
            match_index = getBestMatchIndex(avg, avgs, method)
        ...
        for _img1 in target_images:
            match_index = getBestMatchIndexStruct(_img1, input_images)
            output_images.append(input_images[match_index])
	    # user feedback 
            if count > 0 and batch_size > 10 and count % batch_size is 0: 
                print('processed %d of %d...' %(count, len(target_images))) 
...
def main(): 
    ...
    parser.add_argument('--method', default='mse', required=False)
    ...
    # create photomosaic 
    mosaic_image = createPhotomosaic(target_image, input_images, grid_size,reuse_images, args.method) 
